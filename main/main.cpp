 ...
 * использовать часть этого контроллера, которая реализует последовательный порт
 * (CDC), для работы последовательной консоли вместо того, чтобы использовать
 * для этой цели UART вместе с внешней микросхемой моста USB-UART (наподобие
 * CH340x, FT2232RL и т. п.).
 *
 * ESP32-C3 содержит на кристалле такой контроллер USB, предоставляющий
 * следующие функции: • Двунаправленная последовательная консоль, которую можно
 * использовать как IDF Monitor или другой serial-монитор для отладочных
 * сообщений. • Прошивка FLASH с помощью утилиты esptool.py и команды idf.py
 * flash [2]. • Отладка JTAG с использованием например OpenOCD, одновременно с
 * работой serial-консоли.
 *
 * Обратите внимание, что в отличие от некоторых чипов Espressif с периферийным
 * устройством USB OTG, контроллер USB Serial/JTAG это устройство с
 * фиксированной функцией, реализованной полностью аппаратно. Это значит, что
 * USB Serial/JTAG Controller нельзя переконфигурировать на использование других
 * функций, кроме как каналов обмена последовательными сообщениями (USB CDC) и
 * отладки JTAG.
 */

 /*
 * USB Serial - отладочный вывод и конфигурация с помощью usb_serial_jtag_driver_install
 * UART - Modbus RTU RS485 
 * WiFi - Html status, Modbus TCP, SNMP UPS
 */

#define BUF_SIZE (1024)
#define ECHO_TASK_STACK_SIZE (4096)

static TaskConfig config;
static TaskADC  adc(config);
static TaskServer server(config, &adc);

extern "C" int log_logger(const char *fmt, va_list args) {
  char msg_to_send[300];
  const size_t str_len = vsnprintf(msg_to_send, 299, fmt, args);
  usb_serial_jtag_write_bytes(msg_to_send, str_len, 20 / portTICK_PERIOD_MS);
  return str_len;
}

RtosTaskPool TaskPool(10);
...
extern "C" void app_main(void) {
  ...
  esp_log_level_set("Stat", ESP_LOG_DEBUG);

  // esp_log_set_vprintf(log_logger);
  ...
  esp_log_set_vprintf(log_logger);
  ESP_LOGI("MAIN", "USB_SERIAL_JTAG init done");

  // config = new TaskConfig();
  
  TaskPool.Create(&config);
  TaskPool.Create(&adc);
  TaskPool.Create(&server);
